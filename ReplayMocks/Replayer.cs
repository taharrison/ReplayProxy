...
{
    public class Replayer<T> where T : class
    {
        ...
        private CallReplayingInterceptor _interceptor;

        public Replayer(History history)
        {
            var proxyGen = new Castle.DynamicProxy.ProxyGenerator();
            _interceptor = new CallReplayingInterceptor(history);
